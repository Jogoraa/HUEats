...
const SignupScreen = ({ navigation }) => {
            ...
            firebase.auth().createUserWithEmailAndPassword(email, password)
                .then((userCredentials) => {
                    ...
                    if (userCredentials?.user.uid != null) {
                        const userRef = firebase.firestore().collection('DriverData')
                        userRef.add(
                            {
                                email: email,
                                password: password,
    ...
    return (
        <View style={styles.container}>
            {successmsg == null ?
                <View style={styles.container}>
                    <Text style={styles.head1}>Driver Sign Up</Text>
                    ...
                    <Text style={styles.username}>Please choose username</Text>
                    <View style={styles.inputout} >
                        <TextInput style={styles.input1} placeholder="Enter your Username" onChangeText={(text) => setUsername(text)}
                            onPress={() => {
                                setEmailfocus(false)
                                setPasswordfocus(false)
                                setShowpassword(false)
                    ...
                    <Text >Already have an account?
                        <Text style={styles.signup} onPress={() => navigation.navigate('driverlogin')}> Sign In</Text>
                    </Text>
                </View>
                :
                <View style={styles.container1}>
                    <Text style={styles.successmessage}>{successmsg}</Text>
                    <TouchableOpacity style={btn1} onPress={() => navigation.navigate('driverlogin')}>
